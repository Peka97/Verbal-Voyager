...
if (sendEventUpdateButtonElement) {
  sendEventUpdateButtonElement.addEventListener("click", sendEventUpdate);
}

if (cancelSendEventUpdateButtonElement) {
...
function sendEventUpdate(event) {
  if (sendingBlock) {
    showToast(
      "Данные уже были отправлены ранее. Обновите страницу для отображения актуальной информации и повторите попытку."
    );
    return;
  }
  ...
  fetch(url, data).then((resp) => {
    if (resp.ok) {
      const eventUpdatedCount =
        ...
        Object.keys(dataToSend.lessons).length;
      sendingBlock = true;
...
function getTaskForm(method) {
  ...
  let checklistElement = document.createElement("div", { id: "checklist" });
